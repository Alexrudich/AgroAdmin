...
Для получения токена используется отдельная консольная утилита (см. проект `GoogleTokenHelper` в решении). После однократной авторизации файл сохраняется локально и затем загружается на сервер.
