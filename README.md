...
## 🔔 Мониторинг и поддержание активности

На бесплатном хостинге приложение может "засыпать" при отсутствии активности. Чтобы избежать задержек при работе Telegram-бота, используется внешний мониторинг:

- Эндпоинт `/health` возвращает статус `Healthy` и используется для проверки работоспособности
- Настроен **UptimeRobot** (или аналогичный сервис) с интервалом 15 минут
- Мониторинг поддерживает приложение активным, гарантируя моментальный ответ бота

**Проверка мониторинга:**
```bash
curl -I https://agroadmin.runasp.net/health
